...
export async function GET(request: NextRequest) {
    try {
        ...
        const videoMetrics = await db
            ...
            .from(analyticsEvents)
            .where(
                sql`${dateFilter} AND eventType IN ('video_added', 'video_watched')`,
            )
        ...
        const suggestionMetrics = await db
            ...
            .from(analyticsEvents)
            .where(
                sql`${dateFilter} AND eventType IN ('metadata_extract_success', 'suggestion_accepted', 'platform_suggestion_accepted')`,
            )
        ...
        const tagDistribution = await db
            ...
            .from(videos)
            .where(
                sql`${videos.createdAt} >= ${start} AND ${videos.createdAt} <= ${end} AND tags IS NOT NULL`,
            )
        ...
        const totals = {
            totalTokens: tokenMetrics.reduce(
                (sum, m) => sum + (m.totalTokens || 0),
                0,
            ),
            inputTokens: tokenMetrics.reduce(
                (sum, m) => sum + (m.inputTokens || 0),
                0,
            ),
            outputTokens: tokenMetrics.reduce(
                (sum, m) => sum + (m.outputTokens || 0),
                0,
            ),
            videosAdded: videoMetrics
                .filter((m) => m.eventType === 'video_added')
                .reduce((sum, m) => sum + m.count, 0),
            videosWatched: videoMetrics
                .filter((m) => m.eventType === 'video_watched')
                .reduce((sum, m) => sum + m.count, 0),
        }

        const averages = {
            tokensPerVideo:
                totals.videosAdded > 0
                    ? Math.round(totals.totalTokens / totals.videosAdded)
                    : 0,
            inputOutputRatio:
                totals.outputTokens > 0
                    ? (totals.inputTokens / totals.outputTokens).toFixed(2)
                    : '0',
            acceptanceRate:
                suggestionMetrics[0]?.totalSuggestions > 0
                    ? (
                          (suggestionMetrics[0].acceptedSuggestions /
                              suggestionMetrics[0].totalSuggestions) *
                          100
                      ).toFixed(1)
                    : '0',
        }
        ...
        // Group by platform
        const platformMetrics = videoMetrics.reduce(
            (acc, metric) => {
