...
export async function GET(request: NextRequest) {
    try {
        ...
        const tagWatchData = await db
            ...
            .leftJoin(
                analyticsEvents,
                sql`${videos.id} = (${analyticsEvents.eventData}->>'videoId')::int AND ${analyticsEvents.eventType} = 'video_watched'`,
            )
            .where(
                sql`${videos.createdAt} >= ${start} AND ${videos.createdAt} <= ${end}`,
            )
        ...
        const tagTendencies = tagWatchData.map((tag) => ({
            ...
            watchPercentage:
                tag.videosWithTag > 0
                    ? Math.round((tag.watchedVideos / tag.videosWithTag) * 100)
                    : 0,
        }))
        ...
        const hourlyActivity = hotHours.reduce(
            (acc, hour) => {
        ...
        // Group daily trends
        const dailyActivity = dailyTrends.reduce(
            (acc, day) => {
