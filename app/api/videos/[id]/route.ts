  ...
  try {
    ...
    try {
      if (Array.isArray(result[0].tags)) {
        ...
        parsedTags = result[0].tags && result[0].tags.trim() !== '' ? JSON.parse(result[0].tags) : [];
      } else {
...
export async function PUT(
  ...
  try {
    ...
    const videoWithTags = { ...result[0], tags: [] as Array<{ id: number; name: string; color: string | null }> };
