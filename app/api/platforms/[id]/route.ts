...
export async function PUT(
  ...
  try {
    ...
    if (currentPlatform.isPreset) {
      const restrictedFields = ['platformId', 'name', 'patterns', 'extractor', 'isPreset'];
      const hasRestrictedUpdate = restrictedFields.some(field => updates[field] !== undefined);

      if (hasRestrictedUpdate) {
        return NextResponse.json(
          { success: false, error: 'Cannot modify core fields of preset platforms' },
          { status: 403 }
        );
      }
    }
    ...
    if (updates.patterns) {
      ...
      const validPatterns = updates.patterns.filter((p: any) => typeof p === 'string' && p.trim());
      if (validPatterns.length === 0) {
        return NextResponse.json(
          { success: false, error: 'At least one valid pattern is required' },
          { status: 400 }
        );
      }
