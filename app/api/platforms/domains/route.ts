...
export async function GET(request: NextRequest) {
  try {
    ...
    const matchingPlatform = results.find(platform =>
      platform.patterns && platform.patterns.some(pattern =>
...
export async function POST(request: NextRequest) {
  try {
    ...
    const existingMapping = existing.find(config =>
      config.patterns && config.patterns.some(pattern =>
        pattern.includes(domain) || domain.includes(pattern)
      )
    );
    ...
    if (existingMapping) {
      ...
      await db
        ...
        .set({
          ...
          confidenceScore: Math.max(Number(existingMapping.confidenceScore), confidence).toString(),
          updatedAt: new Date(),
        })
