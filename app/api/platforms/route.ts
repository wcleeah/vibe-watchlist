...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!config.platformId || !config.name || !config.displayName || !config.patterns) {
      return NextResponse.json(
        { success: false, error: 'Missing required fields: platformId, name, displayName, patterns' },
        { status: 400 }
      );
    }
    ...
    if (!Array.isArray(config.patterns)) {
      return NextResponse.json(
        { success: false, error: 'patterns must be an array of strings' },
        { status: 400 }
      );
    }
    ...
    if (existing.length > 0) {
      return NextResponse.json(
        { success: false, error: 'Platform with this ID already exists' },
        { status: 409 }
      );
    }
    ...
    const result = await db.insert(platformConfigs).values(newPlatform).returning();
