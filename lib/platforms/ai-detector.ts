...
export class AIDetector {
  ...
  async initialize() {
    try {
      ...
      console.log(`Loaded ${presets.length} preset platforms for AI detection`);
    } catch (error) {
    ...
    try {
      ...
      if (aiSuggestion && aiSuggestion.confidence > 0.6) {
        ...
        const existingPlatform = await db
          .select()
          .from(platformConfigs)
          .where(eq(platformConfigs.platformId, aiSuggestion.platform))
          .limit(1);
        ...
        return {
          ...
          config: {
            ...
            displayName: aiSuggestion.platform.charAt(0).toUpperCase() + aiSuggestion.platform.slice(1),
  ...
  async registerNewPlatform(suggestion: PlatformSuggestion): Promise<boolean> {
    try {
