...
import type { ParsedUrl } from '@/lib/utils/url-parser'
    ...
    const handleSetManualMode = useCallback(
        (mode: boolean) => {
            ...
            if (mode) {
                ...
                setManualTitle(
                    (prev) => prev || selectedSuggestion?.title || '',
                )
                setManualThumbnailUrl(
                    (prev) => prev || selectedSuggestion?.thumbnailUrl || '',
                )
            } else {
                // Switching to auto: clear manual inputs if they match AI suggestion
                if (manualTitle === selectedSuggestion?.title)
                    setManualTitle('')
                if (manualThumbnailUrl === selectedSuggestion?.thumbnailUrl)
                    setManualThumbnailUrl('')
            }
