...
export function MetadataSelector({
  ...
  if (error) {
    return (
      <div className={cn("p-3 border rounded-lg bg-red-50 border-red-200", className)}>
  ...
  if (suggestions.length === 0) {
    return (
      <div className={cn("p-3 border rounded-lg bg-yellow-50 border-yellow-200", className)}>
        <div className="flex items-center gap-2">
          <AlertCircle className="w-4 h-4 text-yellow-500" />
          <span className="text-sm text-yellow-700">No video content detected</span>
        </div>
        {onManualEdit && (
          <Button
            variant="outline"
        ...
        <div className="flex items-center gap-3 flex-1 min-w-0">
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-2 mb-1">
              <h4 className="font-medium text-sm truncate" title={selectedSuggestion?.title}>
                {selectedSuggestion?.title || 'Select metadata'}
              </h4>
              {selectedSuggestion && (
                <span className={cn(
                  "px-2 py-0.5 text-xs rounded-full border flex items-center gap-1",
                  getConfidenceColor(selectedSuggestion.confidence)
                )}>
                  {getConfidenceIcon(selectedSuggestion.confidence)}
            ...
            {selectedSuggestion && (
              <div className="text-xs text-gray-500 flex items-center gap-2">
                ...
                {selectedSuggestion.reasoning && (
                  <span className="truncate max-w-32" title={selectedSuggestion.reasoning}>
                    • {selectedSuggestion.reasoning}
                  </span>
                )}
              </div>
          ...
          {suggestions.map((suggestion, index) => (
            ...
            >
              <div className="flex items-center gap-3">
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 mb-1">
                    <h5 className="font-medium text-sm truncate" title={suggestion.title}>
                      {suggestion.title}
                    </h5>
                    <span className={cn(
                      "px-2 py-0.5 text-xs rounded-full border flex items-center gap-1",
                  ...
                  <div className="text-xs text-gray-500 flex items-center gap-2">
                    ...
                    {suggestion.reasoning && (
                      <span className="truncate max-w-24" title={suggestion.reasoning}>
                        • {suggestion.reasoning}
                      </span>
                    )}
                  </div>
