...
export function PlatformCreator({ onPlatformCreated }: PlatformCreatorProps) {
  ...
  const addPattern = () => {
    if (formData.currentPattern.trim() && !formData.patterns.includes(formData.currentPattern.trim())) {
      setFormData(prev => ({
        ...prev,
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!formData.platformId.trim() || !formData.name.trim() || !formData.displayName.trim() || formData.patterns.length === 0) {
    ...
    try {
      ...
      if (response.ok) {
        ...
        toast.success(`Platform "${formData.displayName}" created successfully!`);
      ...
      <CardContent>
        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <Input
                ...
                onChange={(e) => setFormData(prev => ({ ...prev, platformId: e.target.value.toLowerCase().replace(/\s+/g, '') }))}
          ...
          <div>
            ...
            <div className="flex gap-2 mb-2">
              <Input
                ...
                onChange={(e) => setFormData(prev => ({ ...prev, currentPattern: e.target.value }))}
              ...
              <Button
                ...
                size="sm"
                onClick={addPattern}
                disabled={!formData.currentPattern.trim() || isSubmitting}
              >
          ...
          <div className="flex gap-2 pt-4">
            ...
            <Button
              type="submit"
              disabled={isSubmitting || !formData.platformId.trim() || !formData.name.trim() || !formData.displayName.trim() || formData.patterns.length === 0}
              className="flex-1"
            >
              {isSubmitting ? (
                <>
